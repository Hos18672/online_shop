...
const ProductPage = () => {
  ...
  const handleWishlistToggle = () => {
    if (isInWishlist) {
      ...
      addToWishlist({ id: product.id, name: product[`name_${lang}`] || product.name_en, price: product.price });
      setShowWishlistAnim(true);
      setTimeout(() => setShowWishlistAnim(false), 1000); // Animation duration
    }
  };

  const productName = product[`name_${lang}`] || product.name_en || t('unnamed');

  return (
    <div className="product-page" role="main" aria-label={t('product_details')} dir={isRtl ? 'rtl' : 'ltr'}>
      <Helmet>
        <title>{productName} - E-commerce</title>
        <meta name="description" content={product[`description_${lang}`] || product.description_en || t('no_description')} />
      </Helmet>
      <div className="product-container">
        ...
        <div className="product-info">
          ...
          <p className="product-description">{product[`description_${lang}`] || product.description_en || t('no_description')}</p>
          <p className="product-price">{t('price')}: ${product.price?.toFixed(2) || 'N/A'}</p>
          ...
          <div className="product-actions">
            ...
            <button
              ...
              aria-label={isInWishlist ? `${t('remove_from_wishlist')} ${productName}` : `${t('add_to_wishlist')} ${productName}`}
              type="button"
            >
              <svg
