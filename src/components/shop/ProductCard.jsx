...
const ProductCard = ({ product }) => {
  ...
  return (
    <div className="product-card" role="article" aria-labelledby={`product-${product.id}`} dir={isRtl ? 'rtl' : 'ltr'}>
      <Link to={`/product/${product.id}`} className="product-card__link">
        ...
        <h3 id={`product-${product.id}`} className="product-card__title">{productName}</h3>
        <p className="product-card__price">${product.price?.toFixed(2) || 'N/A'}</p>
        <p className={`product-card__stock ${product.stock > 0 ? '' : 'out-of-stock'}`}>
